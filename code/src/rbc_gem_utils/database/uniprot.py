...
UNIPROT_ID_RE = re.compile(r"[OPQ][0-9][A-Z0-9]{3}[0-9]|[A-NR-Z][0-9]([A-Z][A-Z0-9]{2}[0-9]){1,2}")
UNIPROT_ISOFORM_ID_RE = re.compile("|".join([s + "-[0-9]{1,2}" for s in UNIPROT_ID_RE.pattern.split("|")]))
...
UNIPROT_QUERY_LABEL_MIRIAM = {
    ...
    # "xref_wbparasite": ("WBParaSite", "xref_wbparasite"),
    ## Organism-specific
    ...
    "xref_phylomedb": ("PhylomeDB", "phylomedb"),
...
def query_UniProt(query_ids, query_parameters, from_db, to_db="UniProtKB", return_failed=True):
...
def get_version_UniProt():
    ...
    response = requests.get("https://ftp.uniprot.org/pub/databases/uniprot/relnotes.txt")
...
def get_annotation_to_from_db_UniProt(miriam_only=True):
    ...
    response = requests.get(
        f"{API_URL}/database/stream?compressed=False&format=json&query=%28*%29"
    )
...
def parse_isoforms_UniProt(df_isoforms, add_canonical=False, fill_missing_isoform=False):
    ...
    for idx, (uniprot_id, uniprot_isoform_value) in df_isoforms.loc[:, ["uniprot", "uniprot.isoform"]].iterrows():
        num_isoforms = re.search(r"Named isoforms=(\d+);", uniprot_isoform_value)
        isoforms = []
        canonical = uniprot_id
        if num_isoforms:
            expected_num = int(num_isoforms.group(1))
            for isoform in re.finditer(f'IsoId=(?P<isoid>{UNIPROT_ISOFORM_ID_RE.pattern})', uniprot_isoform_value):
                isoform_id = isoform.group("isoid")
                if not isoform_id.startswith(uniprot_id):
                    LOGGER.info("Isoform is an alternate entry, decreasing expected number.", len(isoforms), expected_num)
                    expected_num -= 1
                    continue
